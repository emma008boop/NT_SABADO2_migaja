...
def limpiar_datos(df_sucio):
    ...
    df_limpio["tipoGasto"] = df_limpio["tipoGasto"].where(
        df_limpio["tipoGasto"].isin(valores_validos_gastos), pd.NA
    )

    df_limpio["id"] = pd.to_numeric(df_limpio["id"], errors="coerce")
    df_limpio["impactoFinanciero"] = pd.to_numeric(
        df_limpio["impactoFinanciero"], errors="coerce"
    )
    ...
    columnas_obligatorias = ["id", "monto", "tipoGasto"]
